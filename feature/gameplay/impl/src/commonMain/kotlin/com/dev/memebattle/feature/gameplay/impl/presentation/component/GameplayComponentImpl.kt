...
import com.dev.memebattle.feature.gameplay.impl.presentation.store.GameplayStoreFactory
...
class GameplayComponentImpl(
    ...
    private val store = GameplayStoreFactory(storeFactory).create()

    private val labelsFlow = store.labels.shareIn(scope, SharingStarted.Eagerly, replay = 0)
    override val effects: SharedFlow<GameplayStore.Effect> = labelsFlow

    @OptIn(ExperimentalCoroutinesApi::class)
    override val state: StateFlow<GameplayStore.State> = store.stateFlow(scope)
    ...
    override fun onIntent(intent: GameplayStore.Intent) = store.accept(intent)
}
