...
class GameApiServiceImpl(
  ...
  override suspend fun listActiveGames(): NetworkResult<ActiveGamesResponseDto> = safeCall {
    client.get("/games") {
    }
  ...
  override suspend fun getLobbiesWsToken(): NetworkResult<LobbiesWsTokenDto> = safeCall {
    client.get("/games/catalog/ws-token") {
    }
    .body<com.dev.memebattle.core.network.call.BaseResponse<LobbiesWsTokenDto>>().data
  }

  override suspend fun getWsToken(id: String): NetworkResult<WsTokenDto> = safeCall {
    client.get("/games/events/$id/ws-token") {
    }
  ...
  override suspend fun listUserMemePacks(): NetworkResult<List<MemePackDetailsResponse>> =
      safeCall {
    client.get("/games/packs/memes/me") {
    }
    .body<com.dev.memebattle.core.network.call.BaseResponse<List<MemePackDetailsResponse>>>().data
  }
  ...
  override suspend fun listUserSituationPacks(): NetworkResult<List<SituationPackDetailsResponse>> =
      safeCall {
    client.get("/games/packs/situations/me") {
    }
    .body<com.dev.memebattle.core.network.call.BaseResponse<List<SituationPackDetailsResponse>>>().data
  }
  ...
  override suspend fun submitCard(id: String, body: SubmitCardRequest): NetworkResult<Unit> =
      safeCall {
    client.post("/games/$id/submit") {
      setBody(body)
    }
    .let { Unit }
  }

  override suspend fun voteCard(id: String, body: VoteRequest): NetworkResult<Unit> = safeCall {
    client.post("/games/$id/vote") {
      setBody(body)
    }
    .let { Unit }
  }
